...
public class AgendaGenerator {
	...
	private static String renderTask(Project p, CalendarDate date, TaskList tl, Task t, int level,
		...
		if (tl.hasSubTasks(t.getID())) {
			...
			if (expandedTasks.contains(t.getID())) {
				// Util.debug("Task " + t.getID() + " is in list of expanded
				// tasks");
				subTaskOperation = "<a href=\"memoranda:closesubtasks#" + t.getID() + "\">(-)</a>";
			} else {
				// Util.debug("Task " + t.getID() + " is not in list of expanded
				// tasks");
		 ...
		 * String days = ""; if (numOfDays > 1) days = Local.getString("in")+" "
		 * +numOfDays+" "+Local.getString("day(s)"); else days =
		 * Local.getString("tomorrow"); s += "<p>" + Local.getString("Deadline")
		 * + ": <i>" + t.getEndDate().getMediumDateString() + "</i> ("
		 * +days+")</p>"; } } =======
		 */
	...
	static String generateStickers(CalendarDate date) {
		...
		String s = "<hr><hr><table border=\"0\" cellpadding=\"0\" width=\"100%\"><tr><td><a href=\"memoranda:importstickers\"><b>"
				+ Local.getString("Import annotation") + "</b></a></td><td><a href=\"memoranda:exportstickerst\"><b>"
				+ Local.getString("Export annotation as .txt") + "</b></a><td><a href=\"memoranda:exportstickersh\"><b>"
				+ Local.getString("Export annotation as .html") + "</b></a></td></tr></table>"
