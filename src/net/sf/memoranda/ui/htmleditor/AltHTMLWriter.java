...
public class AltHTMLWriter extends AbstractWriter {
	...
	protected void output(char[] chars, int start, int length) throws IOException {
		...
		for (int counter = start; counter < length; counter++) {
			...
			default:
				...
				/**
				 * [alex]I've replaced the following line to avoid to substitute
				 * non-ascii characters by numeric values by default. Use
				 * _nument=true if you need that substitution Original "if"
				 * condition: if (chars [counter]< ' ' || chars [counter] > 127)
				 */
	 ...
	 * ListSelectionModel interface, allowing for it to store state relevant to
	 * a SELECT form element which is implemented as a List. If SELECT has a
	 * size attribute whose value is greater than 1, or if allows multiple
	 * selection then a JList is used to represent it and the OptionListModel is
	...
	class OptionListModel extends DefaultListModel implements ListSelectionModel, Serializable {
		...
		private void fireValueChanged() {
			...
			/*
			 * Change the values before sending the event to the listeners in
			 * case the event causes a listener to make another change to the
			 * selection.
			 */
		...
		private void clear(int r) {
			 ...
			 * minIndex) is not possible because r'th value was set. We only
			 * need to check for the case when lowest entry has been cleared,
			 * and in this case we need to search for the first value set above
			 * it.
			 */
			if (r == minIndex) {
			 ...
			 * maxIndex) is not possible because r'th value was set. We only
			 * need to check for the case when highest entry has been cleared,
			 * and in this case we need to search for the first value set below
			 * it.
			 */
			if (r == maxIndex) {
			 ...
			 * changeSelection() but we will only iterate in the loops above on
		 ...
		 * leadAnchorNotificationEnabled is true the model generates
		 * notification events with bounds that cover all the changes to the
		 * selection plus the changes to the lead and anchor indices. Setting
		 * the flag to false causes a norrowing of the event's bounds to include
		...
		/*
		 * Change the selection with the effect of first clearing the values in
		 * the inclusive range [clearMin, clearMax] then setting the values in
		 * the inclusive range [setMin, setMax]. Do this in one pass so that no
		 * values are cleared if they would later be set.
		 */
		 ...
		 * index is itself selected, set all of the newly inserted items,
		...
		public void insertIndexInterval(int index, int length, boolean before) {
			 ...
			 * will appear at insMaxIndex
			 */
			int insMinIndex = (before) ? index : index + 1;
			int insMaxIndex = (insMinIndex + length) - 1;

			/*
			 * Right shift the entire bitset by length, beginning with index-1
			 * if before is true, index+1 if it's false (i.e. with insMinIndex).
			 */
		...
		/**
		 * Set the anchor selection index, leaving all selection values
		 * unchanged.
		 *
		...
		/**
		 * Set the lead selection index, ensuring that values between the anchor
		 ...
		 * the range [anchor, oldLeadIndex], then select all the values values
		 * in the range [anchor, newLeadIndex], where oldLeadIndex is the old
		 * leadIndex and newLeadIndex is the new one.
		 * <p>
		 * If the value at the anchor index is not selected, do the same thing
		 * in reverse, selecting values in the old range and deslecting values
		 * in the new one.
		 * <p>
		...
		/**
		 * Fetches the BitSet that represents the initial set of selected items
		 * in the list.
		 */
	 ...
	 * Once a Map has been created, and any number of areas have been added, you
	 * can test if a point falls inside the map via the contains method.
	 *
	 * @author Scott Violet
	...
	class Map {
		...
		/**
		 * An array of RegionContainments, will slowly grow to match the length
		 * of areaAttributes as needed.
		 */
		 ...
		 * <code>height</code> gives the size of the region the map is defined
		 * over. If a matching area is found, the AttribueSet for it is
		 * returned.
		 */
		...
		/**
		 * Creates and returns an instance of RegionContainment that can be used
		 * to test if a particular point lies inside a region.
		 */
