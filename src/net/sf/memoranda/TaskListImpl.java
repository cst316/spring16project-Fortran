...
public class TaskListImpl implements TaskList {
	...
	/*
	 * Hastable of "task" XOM elements for quick searching them by ID's (ID =>
	 * element)
	 */
	...
	/**
	 * All methods to obtain list of tasks are consolidated under getAllSubTasks
	...
	private Element getTaskElement(String id) {
		...
		/*
		 * Nodes nodes = XQueryUtil.xquery(_doc, "//task[@id='" + id + "']"); if
		 * (nodes.size() > 0) { Element el = (Element) nodes.get(0); return el;
		 * } else { Util.debug("Task " + id + " cannot be found in project " +
		 * _project.getTitle()); return null; }
		 */
	...
	private boolean isActive(Task t, CalendarDate date) {
		if ((t.getStatus(date) == Task.ACTIVE) || (t.getStatus(date) == Task.DEADLINE)
