...
public class ResourcesListImpl implements ResourcesList {
	 ...
	 * Element("resource"); el.addAttribute(new Attribute("id",
	 * Util.generateId())); el.addAttribute(new Attribute("path", path)); if
	 * (taskId != null) el.addAttribute(new Attribute("taskId", taskId));
	 ...
	 */
	/*
	 * public Vector getResourcesForTask(String taskId) { Vector v = new
	 * Vector(); Elements rs = _root.getChildElements("resource"); for (int i =
	 * 0; i < rs.size(); i++) if
